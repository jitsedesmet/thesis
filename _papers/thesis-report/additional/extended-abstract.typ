...
Luckily, legislative measures have been taken to protect society from this centralization~@bib:gdpr @bib:ccpa.
...
Reading data in this context has already gained some scientific attention @bib:hartig2016walking @bib:taelman-structure-assumptions, but effectively writing data remains rather unexplored.
...
The effectiveness of reading data in a decentralized environment has been increased by abstracting data reads through a query abstraction layer, the query engine, by using query languages like GraphQL~@bib:graphql and SPARQL~@bib:sparql.
...
// Solid uses RDF & LDP 
The Solid specification~@bib:solid-spec builds on top of existing Semantic Web technologies such as RDF (Resource Description Framework)~@bib:rdf and LDP (Linked Data Platform)~@bib:ldp.
...
In this work, we will try to store RDF resources, defined as the CBD (Concise Bounded Description)~@bib:concise-bounded-description of a Named Node.
...
As an example, @fig:rdf-example contains some RDF data in turtle~@bib:turtle format, taking the CBD of named node `<me>` results in @fig:cbd-example. 
...
A self-descriptive declaration of ShEX can be found in @fig:shex-example.
...
Unfortunately, neither Type Indexes~@bib:type-index, nor Shape Trees~@bib:shape-tree are sufficiently descriptive to assess whether a resource should be stored in a document.
...
This exposes pods containing LDBC Social Network Benchmark (SNB)~@bib:ldbc data.
...
There is merit in investigating different interfaces for the use of decentralized data storages, as document oriented interfaces come with drawbacks~@bib:whats-in-pod.
...
A query engine, just like any update API, has the power to choose where to position themselves within the CAP (Consistency, Availability, Partition tolerance) space~@bib:cap. CAP essentially says you can only have two of three properties of {C, A, P} with the choice of a distributed system either being the ACID~@bib:acid or BASE~@bib:base properties.

When choosing the BASE properties, a user chooses to drop consistency.
One way of doing so is by creating CRDTs (Conflict-free Replicated Data Types)~@bib:crdt.
...
This does not mean that we completely drop the availability property, as the inventors of the CAP theorem later describe~@bib:continous-cap.
...
We concluded that the execution time is less than four times slower in the case that the behaviour of our solution could be modelled using a SPARQL query.
